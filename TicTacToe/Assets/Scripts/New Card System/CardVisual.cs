...
public class CardVisual : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    // Cache references for IsSelected optimization
    ...
    private void Awake()
    {
        ...
        // Safe fallback for scaleTarget
        if (scaleTarget == null)
        {
            if (transform.parent != null)
                scaleTarget = transform.parent;
            else
                scaleTarget = transform; // fallback para ele mesmo
    ...
    public void MoveToPosition(Vector3 newPosition)
    {
        ...
        moveTween = transform.DOLocalMove(positionTarget, moveDuration)
            ...
            .OnComplete(() => positionTarget = transform.localPosition); // keep target sync
    }
    ...
    public void OnPointerEnter(PointerEventData eventData)
    {
        if (!IsSelected() && !isDraggingVisual)
        {
            ...
            shakeTween = transform.DOShakePosition(
                ...
                hoverShakeStrength,
                hoverShakeVibrato,
                hoverShakeRandomness,
                fadeOut: true); // shake effect with fade out on hover
        }
    }
    ...
    public void OnBeginDragVisual()
    {
        ...
        scaleTween?.Kill();
        scaleTween = scaleTarget.DOScale(dragScale, 0.2f).SetEase(Ease.OutBack); // smooth scale up
    ...
    public void OnEndDragVisual()
    {
        ...
        scaleTween?.Kill();
        scaleTween = scaleTarget.DOScale(1f, dragScaleDuration).SetEase(Ease.InOutSine); // smooth scale back
        scaleTarget.DOLocalRotate(Vector3.zero, 0.2f).SetEase(Ease.OutCubic);           // smooth rotation reset
    }
    ...
    {
        // garante que não fique nenhum tween ativo em objeto destruído
        KillAllTweens();
    }
}
